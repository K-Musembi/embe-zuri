...
def upload():
    ...
    if image:
        ...
        image_tensor = pd.img_to_tensor(image)
        preprocessed_image = pd.preprocessing(image_tensor)
...
"""@app_views.route("/diagnosis", strict_slashes=False)
def diagnosis():
    comment - leaf image diagnosis
    image_tensor = pd.img_to_tensor(g.image)
    preprocessed_image = pd.preprocessing(image_tensor)
    predicted_disease = pd.predict(preprocessed_image)

    return render_template("results.html", name=predicted_disease)"""
